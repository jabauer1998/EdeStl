...
public class VectorVal implements Value{
    ...
    public boolean boolValue(){
        ...
        public void setValue(Value exp){
            VectorVal vec = exp.asVector();
            int size = Math.min(this.getSize(), vec.getSize());
            int thisStart = this.getStart();
            int vecStart  = vec.getStart();
            for (int j = 0; j < size; j++) {
                ((RegVal)this.getValue(thisStart + j)).setSignal(vec.getValue(vecStart + j).getStateSignal());
            }
        }
